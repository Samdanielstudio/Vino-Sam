  ...
  return (
    <div className="min-h-screen bg-[#E4EBDD] flex flex-col items-center justify-center p-4 md:p-8 font-sans relative overflow-hidden">
      ...
      <div className="max-w-4xl w-full text-center mt-8 md:mt-16 mb-10 md:mb-12">
        <h1 className="text-6xl md:text-[5.5rem] text-[#364736] mb-2 tracking-normal leading-[1.1] font-['Priestacy',_'Great_Vibes',_cursive] font-normal">
          Vinoliya & Samdaniel
        </h1>
            ...
            >
              ...
              {card.dateNum && (
                <div className={`absolute left-6 md:left-8 flex flex-col items-center justify-center text-white z-20 ${!isActive && isMobile ? 'top-1/2 -translate-y-1/2' : 'top-6 md:top-8'}`}>
                  <span className="text-4xl md:text-5xl font-['Cormorant_Garamond'] leading-none font-bold">{card.dateNum}</span>
                  <span className="text-xs md:text-sm tracking-[0.2em] uppercase font-bold mt-1">{card.dateMonth}</span>
                </div>
              )}
              ...
              <div className={`absolute inset-x-4 md:inset-x-8 flex flex-col md:flex-row items-center md:items-end justify-center md:justify-between gap-4 md:gap-0 overflow-hidden z-20 ${!isActive && isMobile ? 'top-1/2 -translate-y-1/2 bottom-auto' : 'bottom-6 md:bottom-8'}`}>
                <AnimatePresence mode="popLayout">
                  {isActive && (
                    ...
                    >
                      <h2 className="text-white text-[1.35rem] leading-[1.2] md:text-[1.6rem] font-semibold md:leading-[1.1]">
                        {card.title}
                      </h2>
                      
                      {/* Additional Details */}
                      {card.time && card.venue && (
                        <div className="flex flex-col items-center md:items-start gap-1 text-white text-sm md:text-base font-['Cormorant_Garamond'] tracking-wide font-medium">
                      ...
                      {card.families && (
                        <div className="flex flex-col items-center md:items-start gap-1 text-white text-sm md:text-base font-['Cormorant_Garamond'] tracking-wider uppercase mt-1 md:mt-2 font-semibold">
                ...
                <motion.div
                  layout
                  className={`text-white flex items-center justify-center whitespace-nowrap z-20 ${isActive ? '' : 'mx-auto'}`}
                  transition={{ duration: 0.5, ease: [0.32, 0.72, 0, 1] }}
                >
                  {card.logo}
                </motion.div>
